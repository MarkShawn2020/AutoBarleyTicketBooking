...
const WECHAT_OFFICIAL_ACCOUNT = "伪装程序大佬(wzcxdl_cs)"

const UrlDetail = "https://detail.damai.cn"
const UrlBuy = "https://buy.damai.cn"
const UrlSeat = "https://seatsvc.damai.cn"
...
$(document).ready(function () {
    ...
    if (cur_url.startsWith(UrlDetail)) {
        console.log("detail")

        let order_url = sessionStorage.getItem('order_url');
    ...
    if (cur_url.startsWith(UrlSeat)) {
        console.log("seat")
        // URLSearchParams接口是浏览器内置的，但这里cur_url应该和location.href一致才对……
        let people_num = new URLSearchParams(window.location.href).get('people_num');
        // todo: 这里要么考虑把people_num改成Number格式，要么就去除判断
        if (people_num === 1) {
            // MutationObserver接口也是浏览器内置的，用于监听元素的变化
...
Number.prototype.toHHMMSS = function () {
    let hours = Math.floor(this / 3600) < 10 ? ("00" + Math.floor(this / 3600)).slice(-2) : Math.floor(this / 3600);
    let minutes = ("00" + Math.floor((this % 3600) / 60)).slice(-2);
    let seconds = ("00" + (this % 3600) % 60).slice(-2);
    return hours + ":" + minutes + ":" + seconds;
};

// 这个函数只用来定时刷新与检测是否开始，过于浪费了，应该还能起到校对时间并修正的作用
function timedUpdate() {
    // 提前2秒开始
    let current_time = Date.now();
    let time_difference = Math.ceil((window.sellStartTime_timestamp - current_time) / 1000);
    if (time_difference < 2) {
        window.location.href = window.order_url;
    } else {
        let time_difference_str = time_difference.toHHMMSS();
        // todo: 然而我并没有在网页中（详情页）找到countdown的id元素
...
function generate_confirm_url(event, price, people_num, data_json) {
    for (let performBase of data_json["performBases"]) {
        for (let perform of performBase["performs"]) {
            if (perform["performName"] === event) {
                // todo: itemId, skuId 等或许可以声明为全局变量
                let itemId = perform["itemId"];
                for (let skuItem of perform["skuList"])
                    if (skuItem["skuName"] === price) {
...
function generate_seat_url(is_calendar, event, price, people_num, data_json) {
    ...
    if (is_calendar) {
        ...
        for (let calendarPerform of data_json["calendarPerforms"]) {
    ...
    let itemId = "";
    for (let performBase of performBases) {
        for (let perform of performBase["performs"]) {
            let performId = perform["performId"];
            let projectId = new URLSearchParams(window.location.href).get('id');
            if (perform["performName"] === event) {
...
function detail_ui() {
    // let $email_input = $('<div class="input_wrapper" id="email_input_wrapper">email：<input id="email_input" type="email" value="example@hotmail.com"></div>');
    // let $name_input = $('<div class="input_wrapper" id="name_input_wrapper">联系人姓名：<input id="name_input" type="text" value="小明"></div>');
    // let $duration_input = $('<div class="input_wrapper" id="duration_input_wrapper">刷新间隔(ms)：<input id="duration_input" type="text" value="5000"></div>');
    let $service = $(".content-right .service");
    let $control_container = $("<div id='control_container'></div>");
    let $wx = $(`<div id="wx" class="notice"><p>公众号：${WECHAT_OFFICIAL_ACCOUNT}</p><p>完整版V-${VERSION}</p></div>`);
    ...
    let $notice2 = $('<div id="notice2" class="notice"><p>倒计时相差1-2秒是四舍五入造成的正常现象</p><p>若误差过大请校准计算机时间，刷新页面</p></div>');
    ...
    $("#start_btn").click(function () {
        ...
        let result = generate_confirm_url(event, price, people_num, data_json);
...
function get_event() {
    let event_css_selector = ".perform__order__select.perform__order__select__performs .select_right_list .active>*";
    return get_text_exclude_children(event_css_selector);

}

function get_price() {
    let price_css_selector = ".select_right_list_item.sku_item.active .skuname";
    return get_text_exclude_children(price_css_selector);
}

function detail_seat_ui() {
    let $notice4 = $('<div id="notice" class="notice warning"><h3>注意</h3><p>若人数为1，选座页面手动选座后自动进入下一步</p><p>人数多于1时，选座页面手动选座后点击“确认选座”按钮或按下空格键进入下一步</p></div>');
    let $service = $(".content-right .service");
    let $control_container = $("<div id='control_container'></div>");
    let $wx = $(`<div id="wx" class="notice"><p>公众号：伪装程序大佬(wzcxdl_cs)</p><p>完整版 ${VERSION}</p></div>`);
    let $number_input = $('<div class="input_wrapper" id="number_input_wrapper">请输入人数：<input id="number_input" type="number" value="1" min="1" max="6"></div>');
    let $start_btn = $('<button id="start_btn">开始抢票</button>');
    let $end_btn = $('<button id="end_btn">停止抢票</button>');
    let $notice = $('<div id="notice" class="notice"><h3>使用步骤</h3><p>0.登录，填写购票人信息</p><p>1.选择场次</p><p>2.选择价格</p><p>3.填写人数</p><p>4.点击‘开始抢票’</p></div>');
    ...
    $control_container.append($end_btn);
    $control_container.append($notice);
    $control_container.append($notice4);
    $control_container.append($notice2);
    ...
    $("#start_btn").click(function () {
        ...
        if ($("#dataDefault").text().includes("calendarPerforms")) {
            let result = generate_seat_url(true, event, price, people_num, data_json);

        } else {
            let result = generate_seat_url(false, event, price, people_num, data_json);
...
function fill_form() {
    ...
    let buyer_number = parseInt($(".ticket-buyer-title em").text());
    window.buyer_number = buyer_number;
    window.curr_buyer = 0;
    console.log(buyer_number);
    let buyer_list = $(".buyer-list-item input");
    for (let i = 0; i < buyer_number; i++) {
